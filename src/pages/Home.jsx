...
export default function Home() {
  // ...existing code...
  // Place the debug marker at the top of the returned JSX
  return (
    <>
      <div style={{background:'#ff0',color:'#000',padding:12,fontWeight:'bold',fontSize:20,zIndex:9999}}>DEBUG: Home component is rendering</div>
      {/* ...existing Home component JSX... */}
    </>
  );
  ...
  const FoundationsContent = ({ isFeminine = false, isAgeless = false, agelessType = null }) => (
    <div style={{marginTop:16,padding:16,borderRadius:12,background:"rgba(218,165,112,0.08)",border:"1px solid rgba(218,165,112,0.12)"}}>
      ...
      {!isAgeless && (
        <div style={{marginBottom:20}}>
          <h5 style={{fontSize:14,color:"var(--champagne)",fontWeight:600,marginBottom:8}}>{foundationalResources.weightAndActivity.title ?? 'Fallback Title'}</h5>
          ...
          <div style={{marginBottom:16,padding:12,borderRadius:8,background:"rgba(230,183,165,0.08)",border:"1px solid rgba(230,183,165,0.12)"}}>
            <p style={{fontSize:13,color:"var(--rosegold)",fontWeight:600,marginBottom:8}}>
              {isFeminine ? (foundationalResources.weightAndActivity.women.title ?? 'Fallback Title') : (foundationalResources.weightAndActivity.men.title ?? 'Fallback Title')}
            </p>
      ...
      {!isAgeless && (
        <div style={{marginBottom:20}}>
          <h5 style={{fontSize:14,color:"var(--champagne)",fontWeight:600,marginBottom:8}}>{foundationalResources.foodServings.title ?? 'Fallback Title'}</h5>
